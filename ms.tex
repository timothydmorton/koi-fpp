...
\kepler\ Object of Interest (KOI).  Out of \ncalc\ KOIs, we determine
that \nval\ have probabilities $<$1\% to be astrophysical false
positives, and thus may be considered validated planets.  \nvalnew\ of
these have not yet been validated or confirmed by other methods.  In
addition, we identify \nfpnew\ KOIs likely to be false positives
($>$90\% \nfpnew) that have not yet been identified as such. A side
product of these calculations is full stellar property posterior
samplings for every host star, modeled as single, binary, and triple
systems.  These calculations use \vespa, a publicly available Python
package able to be easily applied to any transiting exoplanet
candidate.
\end{abstract}
...
reader to \citetalias{Morton:2012} for a detailed description of the
...
of \citet{Bryson:2015}.  \Fig{fppall} shows FPPs for all KOIs passing
...
\item $> 0.99$ probability to be on the target star, according
  to \citet{Bryson:2015}, along with a positional probability
  ``score'' $> 0.4$ (indicating a reliable result), and   
...
nature at the 99\% confidence level.  The reliability of these
calculations is dependent largely on the results of \citet{Bryson:2015},
which quantify the probability that the eclipse/transit signal is
spatially coincident on the sky with the presumed target star.


These batch FPP calculations will 



The \kepler\ mission has demonstrated that space-based transiting
planet surveys can identify viable planet candidates at a rate much
faster than traditional follow-up techniques can confirm them.  As a
result, probabilistic validation techniques are now 
...
\caption{False positive probabilities of all CANDIDATE or CONFIRMED
  ...
  \citep{Bryson:2015}, and have $L_{\rm tot} > 10^{-3}$, indicating
  ...
  \citet{Morton:2011} and \citet{Fressin:2013} \emph{a priori}
...
\caption{A summary of how the the calculations presented in this paper
  ...
  calculations of \citet{Bryson:2015} are not reliable (score $<
  0.4$).  ``On-target probability $<$ 0.99'' means that the positional
