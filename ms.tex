...
signal-to-noise candidates where it is not clear whether the
identified signal is truly transit-like or not must be viewed with
caution.
...
detected via the normal centroid-measuring methods.

Thus there are many KOIs that have
already been identified as false alarms (and thus dispositioned FALSE
POSITIVE at the NExScI archive) for which \vespa\ gives a low false
positive probability---all this means is that the planet scenario for
those KOIs is \emph{more} probable than the other scenarios being
considered, but 



from several pixels or more away, often the result of
the point spread function (PSF) of a bright eclipsing binary star
leaking into the aperture of a nearby target star
\citep{Coughlin:2014}.  Most of these have been 

the limits of the \vespa\ method.  

The first thing to note about these results is that they should not
all be taken at face value.  For example, many KOIs have already been
identified as false positives, and designated 
...
\caption{False positive probabilities of all KOIs for which we
  ...
  $10^{-4}$ for visualization purposes).  This includes
  KOIs that are not dispositioned as FALSE POSITIVE on the NExScI
  archive, are considered to be reliably located on the target star ($
  {\rm Pr} > 0.99$) according to pixel-level analysis
  ...
  \kepler\ candidate false positive rate.
\figlabel{fppall}}
\end{figure*}
