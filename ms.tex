...
gather ancillary data and constraints from various sources (described
below) in order to enable the \vespa\ calculation. For various
reasons, the data are not available for every \ac{koi}, and so in the
end we present \ac{fpp} results for \ncalc\ \acp{koi}.

For each \ac{koi} we obtain the following data:
\begin{enumerate}
...
\item The maximum secondary eclipse depth allowed by the
  ...
  measured transit depth (\verb|koi_depth_err1|).
